...
# objects randoms
var_s_t = (3*t_diff^2)/(18); std_s_t = sqrt(var_s_t);
var_r_s = 2e-5/3;            std_r_s = sqrt(var_r_s);
var_r_a = 1e-5/3;            std_r_a = sqrt(var_r_a); max_r_a = sqrt(3) * std_r_a;
var_r_c = (aq^2)/12;         std_r_c = sqrt(var_r_c);
...
params_A = stk_sampling_sobol(iters_a, 2);

temp = triinv(double(params_A(:,1)), 17, 20, 23) + (rand - 0.5) * 0.01;
phi = double(params_A(:,2)) * T0 + (rand - 0.5) * 0.01 * T0;
...
for i = 1 : iters_a
  ...
  params_B = stk_sampling_randomlhs(ns, 2);
  ...
  # inject signal noise
  ys = y;
  ys = y + norminv(double(params_B(:,1)), 0, std_r_s)';

  # perform converter part tasks
  ya = ys;
  ya = ya + f_tm_a(temp(i));

  ya = ya + 2 * max_r_a * (double(params_B(:,2)) - 0.5)';
  ya = ya ...
